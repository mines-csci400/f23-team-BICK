...
let rec map_inorder (f : 'v -> 'a) (t : 'v binary_tree) : 'a list =
  match t with
  | Empty -> []
  | Node (left, value, right) ->
    let left_value = map_inorder f left in
    let current_value = f value in
    let right_value = map_inorder f right in
    left_value @ [current_value] @ right_value

(* Apply function f to every data element of tree t and collect the
   results in a list following a reverse order traversal of the tree *)
let rec map_revorder (f : 'v -> 'a) (t : 'v binary_tree) : 'a list =
  match t with
  | Empty -> []
  | Node (left, value, right) ->
    let left_value = map_revorder f left in
    let right_value = map_revorder f right in
    let current_value = f value in
    right_value @ [current_value] @ left_value
   ...
   [
     (Some("simple tree"),
      ...
      (identity,
       Node(l2, 3, l4)),
       Ok([2;3;4]));
      (Some("2 same nums first"),
       (identity,
       Node(l1,1,l2)),
       Ok([1;1;2]));
      (Some("2 same nums last"),
       (identity,
       Node(l1,2,l2)),
       Ok([1;2;2]));
      (Some("All the same"),
   ...
   [
     ...
     (Some("simple tree 2"),
      (identity,
       Node(l2, 3, l4)),
       Ok([4;3;2]));
     (Some("same num in front"),
      (identity,
       Node(l1, 1, l2)),
       Ok([2;1;1]));
     (Some("same num in back"),
      (identity,
       Node(l1, 2, l2)),
       Ok([2;2;1]));
     (Some("same nums"),
